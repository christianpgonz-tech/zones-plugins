...
@objc(FootballZone)
public final class FootballZone: NSObject {
    @objc public var zoneIdentifier: String { "nfl-football" }
    @objc public var zoneTitle: String { "NFL Football" }
    @objc public var zoneSymbol: String { "football.fill" }
    @objc public var zonePreferredWidth: NSNumber { 320 }
    @objc public var zonePreferredHeight: NSNumber { 236 }
    /// Zones (0.13.1 and later) shows this as a button under Settings → Added Zones → NFL Football.
    @objc public var zoneSettingsTitle: String { "Football Settings…" }
    @objc public func zoneOpenSettings() { DispatchQueue.main.async { FootballSettingsController.shared.show() } }
    /// Zones uses the first entry as this zone's starting position, so the list is ordered: free spots first.
    @objc public var zoneAllowedPlacements: [String] { FootballZone.placementOrder() }
    ...
    public override init() {
        ...
        DispatchQueue.main.async { FootballStore.shared.start(); FBChip.shared.start() }
    }
    ...
    static func placementOrder() -> [String] {
        ...
        if let saved = d.string(forKey: "fb.defaultPlacement"), preference.contains(saved) { return [saved] + preference.filter { $0 != saved } }
        var used = Set<String>()
        ...
        for (id, on) in enabledAdded where on && id != "added.nfl-football" { used.insert(placedAdded[id] ?? "top") }
        let pick = preference.first { !used.contains($0) } ?? "top"
        d.set(pick, forKey: "fb.defaultPlacement")
    ...
    @objc public func makeViewWithContext(_ context: NSDictionary) -> NSView {
        ...
        let hit = FBHitMap()
        let host = NSHostingView(rootView: FootballView(placement: placement, hit: hit))
        host.sizingOptions = []
        let box = FBPassThroughBox(hit: hit)
