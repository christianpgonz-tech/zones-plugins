...
enum FootballMode { case cards, league, picker }

final class FootballUI: ObservableObject {
    init(mode: FootballMode = .cards) { self.mode = mode }
    @Published var mode: FootballMode
    @Published var cardIndex = 0
    @Published var leagueWeek = 0
    @Published var weekOffset = 0                 // 0 = the current week
    @Published var filter = "all"                // all or mine
...
final class FBHitMap { var contains: (CGPoint) -> Bool = { _ in true } }

final class FBPassThroughBox: NSView {
    let hit: FBHitMap
    init(hit: FBHitMap) { self.hit = hit; super.init(frame: .zero) }
...
struct FootballView: View {
    ...
    init(placement: String, hit: FBHitMap, startMode: FootballMode = .cards) {
        self.placement = placement; self.hit = hit
        _ui = StateObject(wrappedValue: FootballUI(mode: startMode))
    }
    @StateObject private var ui: FootballUI
            ...
            ZStack(alignment: .topLeading) {
                if ui.mode == .picker {
                    ...
                    if ui.mode != .picker, let spot = SportBadgeLayout.spot(geo: geo, rect: rect) {
                        SportBadge(title: "NFL Football", size: spot.size) { Image(systemName: "football.fill").resizable().scaledToFit() }
...
private struct PanelCanvas: View {
    @ObservedObject var ui: FootballUI
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            Group {
                switch ui.mode {
                case .league: LeaguePage(ui: ui, store: store)
    ...
    private var topBar: some View {
        HStack(spacing: 4) {
            ...
            Button { ui.mode = .picker } label: { Image(systemName: "square.grid.3x3.fill").font(.system(size: 11)) }
                .buttonStyle(.plain).help("All 32 teams: pick your favorites")
            Button { FootballSettingsController.shared.show() } label: { Image(systemName: "gearshape").font(.system(size: 12)) }
                .buttonStyle(.plain).help("Football settings")
        }.frame(height: 22)
    }
    private func tab(_ title: String, _ mode: FootballMode) -> some View {
    ...
    private var bottomBar: some View {
        HStack(spacing: 8) {
            arrow("chevron.left", "Previous team") { ui.cardIndex = (ui.cardIndex - 1 + max(1, store.favorites.count)) % max(1, store.favorites.count) }
            Spacer()
            HStack(spacing: 2) {
            ...
            Spacer()
            arrow("chevron.right", "Next team") { ui.cardIndex = (ui.cardIndex + 1) % max(1, store.favorites.count) }
        }.frame(height: 30).opacity(store.favorites.count > 1 ? 1 : 0)
    }
}

/// A large, easy-to-hit arrow button.
private func arrow(_ symbol: String, _ help: String, _ action: @escaping () -> Void) -> some View {
    ...
    var body: some View {
        if store.favorites.isEmpty {
            VStack(spacing: 8) {
                ...
                Text("Choose your favorites and their games rotate here, live.").font(.caption).foregroundStyle(.secondary).multilineTextAlignment(.center)
    ...
    @ViewBuilder private var cards: some View {
        ...
        if let game = store.currentGame(for: abbr) {
            GameCard(game: game, focus: abbr, celebrate: ui.celebration?.team)
                ...
                .onTapGesture { FootballDetailsController.shared.show(team: abbr, game: game) }
                .onHover { ui.hoveringCard = $0 }
                .help("Click for scores, standings and stats")
                .id(abbr + game.id)
                .transition(.opacity)
        } else {
            VStack(spacing: 6) {
...
private struct CelebrationBanner: View {
    ...
    var body: some View {
        ...
        HStack(spacing: 6) {
            ...
            Text(store.team(info.team)?.nick.uppercased() ?? info.team).font(.system(size: 11, weight: .bold)).opacity(0.9)
        }
    ...
    private var footer: some View {
        VStack(spacing: 2) {
            if game.state == .pre {
                ...
                Text("Tap for the season, standings and stats").font(.system(size: 10)).foregroundStyle(.secondary)
            }
        }
    }
}
...
private struct TeamBlock: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 2) {
            ...
            HStack(spacing: 3) {
                Text(team?.nick ?? abbr).font(.system(size: 11, weight: abbr == focus ? .bold : .semibold)).lineLimit(1)
...
// MARK: - Games (every game of a week, the whole NFL)

private struct LeaguePage: View {
    @ObservedObject var ui: FootballUI
    @ObservedObject var store: FootballStore
    private var week: Int { ui.weekOffset == 0 ? store.currentWeek : max(1, min(18, store.currentWeek + ui.weekOffset)) }
    private func games() -> [Game] {
        ...
        }.sorted { ($0.state == .live ? 0 : 1, $0.date) < ($1.state == .live ? 0 : 1, $1.date) }
    }
    var body: some View {
        let list = games()
        VStack(spacing: 6) {
            HStack {
                arrow("chevron.left", "Earlier week") { go(-1) }.disabled(week <= 1)
                Spacer()
                VStack(spacing: 0) {
                ...
                Spacer()
                arrow("chevron.right", "Later week") { go(1) }.disabled(week >= 18)
            }
            HStack(spacing: 4) {
    ...
    private func go(_ d: Int) {
        let target = max(1, min(18, week + d))
...
private struct GameRow: View {
    ...
    var body: some View {
        ...
        HStack(spacing: 5) {
            side(game.away, game.awayScore)
            Text("@").font(.system(size: 9)).foregroundStyle(.secondary)
            side(game.home, game.homeScore)
            Spacer(minLength: 2)
            HStack(spacing: 3) {
        ...
        .contentShape(Rectangle())
        .onTapGesture(count: 2) { FootballDetailsController.shared.show(team: store.favorites.first { game.involves($0) } ?? game.home, game: game) }
