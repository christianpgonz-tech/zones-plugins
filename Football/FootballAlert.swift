...
struct ScoreAlertInfo: Identifiable {
    let id = UUID()
    let kind: String            // "TOUCHDOWN", "FIELD GOAL", "SAFETY"
...
final class FBScoreAlert {
    static let shared = FBScoreAlert()
    ...
    func show(_ info: ScoreAlertInfo) {
        ...
        let view = ScoreAlertView(info: info, seconds: seconds, start: Date(), close: { [weak self] in self?.hide() },
                                  open: { [weak self] in self?.hide(); FootballDetailsController.shared.show(team: info.team, game: info.game) })
        if panel == nil {
            let p = FBAlertPanel(contentRect: NSRect(origin: .zero, size: size), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
    ...
    private func target(_ size: CGSize) -> NSRect {
        let handle = NSApp.windows.first { $0.title == "NFL Football" && $0.isVisible && $0.frame.width <= 96 && $0.frame.height <= 96 }
...
final class FBAlertPanel: NSPanel {
...
struct ScoreAlertView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                ...
                Text(team?.nick.uppercased() ?? info.team).font(.system(size: 14, weight: .bold)).opacity(0.9)
                Spacer()
    ...
    /// The play text from the live feed when it describes the score, otherwise a plain summary.
    private func playText(_ g: Game) -> String {
        let words = ["touchdown", "field goal", "safety", "yard", "pass", "run", "td"]
        if let p = g.lastPlay, words.contains(where: { p.lowercased().contains($0) }) { return p }
