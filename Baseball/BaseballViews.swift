...
enum BaseballMode { case cards, games, picker }

final class BaseballUI: ObservableObject {
    init(mode: BaseballMode = .cards) { self.mode = mode }
    @Published var mode: BaseballMode
...
final class BaseballHitMap { var contains: (CGPoint) -> Bool = { _ in true } }

final class BaseballPassThroughBox: NSView {
    let hit: BaseballHitMap
    init(hit: BaseballHitMap) { self.hit = hit; super.init(frame: .zero) }
...
struct BaseballView: View {
    ...
    init(placement: String, hit: BaseballHitMap, startMode: BaseballMode = .cards) {
        self.placement = placement; self.hit = hit
        _ui = StateObject(wrappedValue: BaseballUI(mode: startMode))
    }
    @StateObject private var ui: BaseballUI
            ...
            ZStack(alignment: .topLeading) {
                if ui.mode == .picker {
                    ...
                    if ui.mode != .picker, let spot = SportBadgeLayout.spot(geo: geo, rect: rect) {
                        SportBadge(title: "MLB Baseball", size: spot.size) {
                            if let icon = BaseballIcon.image { Image(nsImage: icon).renderingMode(.template).resizable().scaledToFit() }
                            else { Image(systemName: "baseball.fill").resizable().scaledToFit() }
...
private struct PanelCanvas: View {
    @ObservedObject var ui: BaseballUI
    @ObservedObject var store: BaseballStore
    var body: some View {
        VStack(spacing: 6) {
            topBar
            Group {
    ...
    private var topBar: some View {
        HStack(spacing: 4) {
            ...
            Button { ui.picker.searchActive = true; ui.mode = .picker } label: { Image(systemName: "magnifyingglass").font(.system(size: 11, weight: .semibold)) }
                .buttonStyle(.plain).help("Search for a team")
            Button { ui.mode = .picker } label: { Image(systemName: "square.grid.3x3.fill").font(.system(size: 11)) }
                .buttonStyle(.plain).help("Every MLB team: pick your favorites")
            Button { BaseballSettingsController.shared.show() } label: { Image(systemName: "gearshape").font(.system(size: 12)) }
                .buttonStyle(.plain).help("MLB Baseball settings")
        }.frame(height: 22)
    }
    private func tab(_ title: String, _ mode: BaseballMode) -> some View {
    ...
    private var bottomBar: some View {
        HStack(spacing: 8) {
            scArrow("chevron.left", "Previous team") { ui.cardIndex = (ui.cardIndex - 1 + max(1, store.favorites.count)) % max(1, store.favorites.count) }
            Spacer()
            HStack(spacing: 2) {
            ...
            Spacer()
            scArrow("chevron.right", "Next team") { ui.cardIndex = (ui.cardIndex + 1) % max(1, store.favorites.count) }
...
private struct CardsPage: View {
    ...
    var body: some View {
        if store.favorites.isEmpty {
            VStack(spacing: 8) {
                Image(systemName: "baseball.fill").font(.system(size: 30)).foregroundStyle(.secondary)
                Text("Pick your teams").font(.headline)
                Text("Pick your favorite teams and their games rotate here, live.").font(.caption).foregroundStyle(.secondary).multilineTextAlignment(.center)
                Button("Choose teams") { ui.mode = .picker }.buttonStyle(.borderedProminent).controlSize(.small)
            }.frame(maxWidth: .infinity, maxHeight: .infinity)
        } else { cards }
    }
    @ViewBuilder private var cards: some View {
        ...
        if let game = store.currentGame(for: abbr) {
            GameCard(game: game, focus: abbr, celebrate: ui.celebration?.team)
                ...
                .onTapGesture { BaseballDetailsController.shared.show(team: abbr, game: game) }
                .onHover { ui.hoveringCard = $0 }
                .help("Click for the season, standings and stats")
                .id(abbr + game.id)
                .transition(.opacity)
        } else {
            VStack(spacing: 6) {
...
struct GameCard: View {
        ...
        VStack(spacing: 4) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    dots("B", game.balls, 4, Color(red: 0.19, green: 0.82, blue: 0.35))
                    dots("S", game.strikes, 3, Color(red: 1, green: 0.8, blue: 0.1))
                    dots("O", game.outs, 3, Color(red: 1, green: 0.27, blue: 0.23))
                }
                VStack(alignment: .leading, spacing: 2) {
                    if let b = game.batter { Text("AB  \(b)").lineLimit(1) }
                    if let p = game.pitcher { Text("P   \(p)").lineLimit(1) }
                }.font(.system(size: 10)).foregroundStyle(.secondary)
                Spacer(minLength: 0)
                if game.risp { Text("RISP").font(.system(size: 9, weight: .bold)).foregroundStyle(.white).padding(.horizontal, 6).padding(.vertical, 2).background(Capsule().fill(Color.red)) }
            }
    ...
    private func dots(_ label: String, _ value: Int, _ count: Int, _ color: Color) -> some View {
        HStack(spacing: 3) {
            Text(label).font(.system(size: 9, weight: .bold)).foregroundStyle(.secondary).frame(width: 9, alignment: .leading)
            ForEach(0..<count, id: \.self) { i in Circle().fill(i < value ? color : Color.primary.opacity(0.18)).frame(width: 7, height: 7) }
        }
    }
    private var footer: some View {
        VStack(spacing: 2) {
            if game.state == .pre {
                ...
                Text("Click for the season, standings and stats").font(.system(size: 10)).foregroundStyle(.secondary)
            }
        }
    }
}
...
private struct TeamBlock: View {
    ...
    @ObservedObject var store = BaseballStore.shared
    var body: some View {
        let hasBall = game.state == .live && game.isBatting(abbr)
        let team = store.team(abbr)
        let record = abbr == game.away ? game.awayRecord : game.homeRecord
        let extra: String? = game.state == .pre ? nil : "\(abbr == game.away ? game.awayHits : game.homeHits) H · \(abbr == game.away ? game.awayErrors : game.homeErrors) E"
        VStack(spacing: 2) {
            ZStack(alignment: .bottomTrailing) {
                ...
                if hasBall {
                    Image(systemName: "baseball.fill").font(.system(size: 11)).foregroundStyle(.white)
                        .padding(4).background(Circle().fill(Color(red: 0.75, green: 0.1, blue: 0.15))).offset(x: 4, y: 2)
                }
            }
            HStack(spacing: 3) {
            ...
            Text(extra ?? " ").font(.system(size: 9)).foregroundStyle(.secondary).lineLimit(1)
        }.frame(width: 90)
    }
}
...
struct TimeBar: View {
    let game: Game
    var body: some View {
        let n = BaseballSport.regulation
        VStack(spacing: 2) {
            GeometryReader { proxy in
            ...
            HStack {
                ForEach(1...n, id: \.self) { q in
                    Text(BaseballSport.periodNames[q - 1]).font(.system(size: 8, weight: game.state == .live && game.period == q ? .bold : .regular))
                        .foregroundStyle(game.state == .live && game.period == q ? Color.primary : .secondary).frame(maxWidth: .infinity)
                }
                if game.period > n { Text(BaseballSport.extraLabel).font(.system(size: 8, weight: .bold)).foregroundStyle(.orange) }
            }
        }
    }
}

/// The bases: a square turns yellow while a runner is on it.
private struct Diamond: View {
    let first: Bool, second: Bool, third: Bool
    var body: some View {
        ZStack {
            base(second).offset(y: -9)
            base(third).offset(x: -12, y: 3)
            base(first).offset(x: 12, y: 3)
        }.frame(width: 42, height: 32)
    }
    private func base(_ on: Bool) -> some View {
        Rectangle().fill(on ? Color.yellow : Color.primary.opacity(0.16)).frame(width: 10, height: 10)
            .overlay(Rectangle().stroke(Color.primary.opacity(0.45), lineWidth: 0.6)).rotationEffect(.degrees(45))
    }
}
...
private struct GamesPage: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 6) {
            ...
            HStack(spacing: 4) {
                chip("All", "all"); chip("My teams", "mine")
                Spacer()
            }
            if list.isEmpty {
...
private struct GameRow: View {
    ...
    var body: some View {
        ...
        .contentShape(Rectangle())
        .onTapGesture(count: 2) { BaseballDetailsController.shared.show(team: store.favorites.first { game.involves($0) } ?? game.home, game: game) }
...
enum BaseballIcon {
    static let image: NSImage? = {
        guard let url = Bundle(for: BaseballZone.self).url(forResource: "ZoneIcon", withExtension: "png") else { return nil }
