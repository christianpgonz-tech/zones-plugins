...
// How MLB Baseball plugs into the shared team picker (../Shared/SportsPicker.swift).
...
extension BaseballStore: PickerStore {
    var pickTeams: [Team] { teams }
    var pickGroups: [PickerGroup] {
        conferences.enumerated().map { i, c in
            PickerGroup(name: c, short: c, color: Color(hue: Double(i) / Double(max(1, conferences.count)), saturation: 0.5, brightness: 0.7))
        }
    }
    var pickRings: Int { 3 }
    ...
    func pickSubtitle(_ key: String) -> String {
        ...
        return "\(t?.conference ?? "")\(s.map { " · \($0.overall)" } ?? "")"
    }
    ...
    func pickOpen(_ key: String) { BaseballDetailsController.shared.show(team: key, game: currentGame(for: key).map { fresh($0) }) }
}
