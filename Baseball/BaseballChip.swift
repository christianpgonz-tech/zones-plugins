...
final class BaseballChip {
    static let shared = BaseballChip()
    private var panel: NSPanel?
    private var timer: Timer?
    private let state = BaseballXChipState()
    ...
    private func handle() -> NSWindow? { NSApp.windows.first { $0.title == "MLB Baseball" && $0.isVisible } }
    ...
    private func tick() {
        ...
        let side = 2 * (R + 34)
        if panel == nil {
            let p = BaseballXChipPanel(contentRect: NSRect(x: 0, y: 0, width: side, height: side), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
            p.isOpaque = false; p.backgroundColor = .clear; p.hasShadow = false; p.ignoresMouseEvents = true
            p.isReleasedWhenClosed = false
            p.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]
            p.contentView = NSHostingView(rootView: BaseballXChipView(state: state))
            panel = p
        }
        ...
        default: let m = max(0, Int(g.date.timeIntervalSinceNow / 60)); label = m > 0 ? "in \(m)m" : "First pitch"
        }
        state.update(placement: placement, radius: R, progress: g.progress, live: g.state == .live,
                     lead: mine > theirs ? 1 : (mine < theirs ? -1 : 0), redZone: g.state == .live && g.risp && g.isBatting(me),
                     hasBall: g.state == .live && g.isBatting(me), label: label)
        panel?.orderFrontRegardless()
    }
}

final class BaseballXChipPanel: NSPanel {
...
final class BaseballXChipState: ObservableObject {
...
struct BaseballXChipView: View {
    @ObservedObject var state: BaseballXChipState
    var body: some View {
        GeometryReader { proxy in
            ...
            let reg = BaseballSport.regulation, n = Double(reg)
            let gap = 0.025, w = geo.span / n
            ...
            TimelineView(.animation(minimumInterval: 1 / 20, paused: !state.redZone)) { ctx in
                ...
                ZStack {
                    ...
                    HStack(spacing: 3) {
                        if state.hasBall { Image(systemName: "baseball.fill").font(.system(size: 8)) }
