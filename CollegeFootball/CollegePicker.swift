...
// How College Football plugs into the shared team picker (../Shared/SportsPicker.swift).
...
extension CollegeStore: PickerStore {
    var pickTeams: [Team] { teams }
    var pickGroups: [PickerGroup] {
        conferences.enumerated().map { i, c in
            PickerGroup(name: c, short: ["Independents": "IND", "Mountain West": "MWC", "American": "AAC"][c] ?? c, color: Color(hue: Double(i) / 11.0, saturation: 0.5, brightness: 0.7))
        }
    }
    var pickRings: Int { 5 }
    ...
    func pickOpen(_ key: String) { CollegeDetailsController.shared.show(team: key, game: currentGame(for: key).map { fresh($0) }) }
}
