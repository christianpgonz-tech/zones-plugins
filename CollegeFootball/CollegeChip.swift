...
final class CFBChip {
    static let shared = CFBChip()
    private var panel: NSPanel?
    private var timer: Timer?
    private let state = CCFBChipState()
    ...
    private func handle() -> NSWindow? { NSApp.windows.first { $0.title == "College Football" && $0.isVisible } }
    ...
    private func tick() {
        ...
        let side = 2 * (R + 34)
        if panel == nil {
            let p = CCFBChipPanel(contentRect: NSRect(x: 0, y: 0, width: side, height: side), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
            p.isOpaque = false; p.backgroundColor = .clear; p.hasShadow = false; p.ignoresMouseEvents = true
            p.isReleasedWhenClosed = false
            p.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]
            p.contentView = NSHostingView(rootView: CCFBChipView(state: state))
            panel = p
        }
...
final class CCFBChipPanel: NSPanel {
...
final class CCFBChipState: ObservableObject {
...
struct CCFBChipView: View {
    @ObservedObject var state: CCFBChipState
    var body: some View {
