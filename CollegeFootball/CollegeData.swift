...
struct Game: Identifiable, Hashable {
    ...
    /// How much of a 60-minute game has been played (overtime counts as full).
    var progress: Double {
        switch state {
        case .pre: return 0
        case .post: return 1
    ...
    var statusLine: String {
        ...
        case .live:
            ...
            return "\(period > 4 ? "OT" : "Q\(period)") \(clock)"
        }
    }
...
/// Everything the zone shows comes from ESPN's public college football feed (free, unofficial): teams and conferences,
...
final class CollegeStore: ObservableObject {
    static let shared = CollegeStore()
    ...
    // Settings (saved)
    @Published var favorites: [String] { didSet { UserDefaults.standard.set(favorites, forKey: "cf.favorites"); loadSchedules() } }
    @Published var showLogos: Bool { didSet { UserDefaults.standard.set(showLogos, forKey: "cf.logos") } }
    @Published var rotateSeconds: Double { didSet { UserDefaults.standard.set(rotateSeconds, forKey: "cf.rotate") } }
    @Published var chipEnabled: Bool { didSet { UserDefaults.standard.set(chipEnabled, forKey: "cf.chip") } }
    /// Score alert: "off", "td" (touchdowns only) or "all" (every scoring play).
    @Published var alertMode: String { didSet { UserDefaults.standard.set(alertMode, forKey: "cf.alertMode") } }
    @Published var alertOthers: Bool { didSet { UserDefaults.standard.set(alertOthers, forKey: "cf.alertOthers") } }
    @Published var alertSeconds: Double { didSet { UserDefaults.standard.set(alertSeconds, forKey: "cf.alertSeconds") } }
    /// "zone" (the zone itself opens bigger), "window" (a separate card), or "both".
    @Published var alertStyle: String { didSet { UserDefaults.standard.set(alertStyle, forKey: "cf.alertStyle") } }
    ...
    private let fixtures: URL? = ProcessInfo.processInfo.environment["CFB_FIXTURES"].map { URL(fileURLWithPath: $0) }
    static var appCanOpenZones: Bool { (Int(Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "") ?? 0) >= 20 }
    private static let site = "https://site.api.espn.com/apis/site/v2/sports/football/college-football"
    private static let apiV2 = "https://site.api.espn.com/apis/v2/sports/football/college-football"
    static let conferenceOrder = ["SEC", "Big Ten", "Big 12", "ACC", "Pac-12", "American", "Sun Belt", "MAC", "Mountain West", "C-USA", "Independents"]

    init() {
        let d = UserDefaults.standard
        favorites = d.stringArray(forKey: "cf.favorites") ?? []
        showLogos = d.object(forKey: "cf.logos") as? Bool ?? true
        rotateSeconds = d.object(forKey: "cf.rotate") as? Double ?? 8
        chipEnabled = d.object(forKey: "cf.chip") as? Bool ?? true
        alertMode = d.string(forKey: "cf.alertMode") ?? "td"
        alertOthers = d.object(forKey: "cf.alertOthers") as? Bool ?? false
        alertSeconds = d.object(forKey: "cf.alertSeconds") as? Double ?? 10
        alertStyle = d.string(forKey: "cf.alertStyle") ?? "zone"
    }
    ...
    static var cacheFolder: URL {
        ...
        let url = base.appendingPathComponent("Zones/CollegeFootball", isDirectory: true)
    ...
    private func fetch(_ urlString: String, cacheName: String, maxAge: TimeInterval, completion: @escaping (Data?) -> Void) {
        ...
        request.setValue("Zones-CollegeFootball/1.0", forHTTPHeaderField: "User-Agent")
    ...
    private static func shortConference(_ name: String) -> String {
        ...
        if n.contains("southeastern") || n == "sec" { return "SEC" }
        if n.contains("big ten") { return "Big Ten" }
        if n.contains("big 12") { return "Big 12" }
        if n.contains("atlantic coast") || n == "acc" { return "ACC" }
        if n.contains("pac-12") { return "Pac-12" }
        if n.contains("mid-american") { return "MAC" }
        if n.contains("american") { return "American" }
        if n.contains("sun belt") { return "Sun Belt" }
        if n.contains("mountain west") { return "Mountain West" }
        if n.contains("conference usa") { return "C-USA" }
        if n.contains("independent") { return "Independents" }
        return name
    }
    ...
    private func loadTeams() {
        ...
        fetch("\(Self.site)/teams?limit=500", cacheName: "teams.json", maxAge: 7 * 24 * 3600) { [weak self] teamData in
            ...
            self.fetch("\(Self.apiV2)/standings?group=80&level=3", cacheName: "standings.json", maxAge: 1800) { data in
                    ...
                    if let entries = (node["standings"] as? [String: Any])?["entries"] as? [[String: Any]] {
                        ...
                        for e in entries {
                            ...
                            teams.append(Team(abbr: abbr, espnID: id, school: school, name: t["displayName"] as? String ?? school, nick: t["name"] as? String ?? "",
                                              conference: conference, colorHex: colors[id] ?? "444444", logoURL: logo))
                        ...
                        if let i = tables.firstIndex(where: { $0.name == conference }) { tables[i].rows += rows } else { tables.append(ConferenceTable(name: conference, rows: rows)) }
                    }
                ...
                tables.sort { (Self.conferenceOrder.firstIndex(of: $0.name) ?? 99) < (Self.conferenceOrder.firstIndex(of: $1.name) ?? 99) }
    ...
    func board(week: Int) -> [Game] { boards[week] ?? [] }
    func refreshBoard() { lastBoardFetch = Date(); loadWeek(nil) }

    /// `week == nil` loads the current week (the one ESPN reports as current).
    func loadWeek(_ week: Int?) {
        if let week, boards[week] != nil, week != currentWeek, (boards[week] ?? []).allSatisfy({ $0.state == .post }) { return }
        var url = "\(Self.site)/scoreboard?groups=80&limit=300"
        if let week { url += "&week=\(week)&seasontype=2" }
    ...
    /// One game from an ESPN event (scoreboard or team schedule); also remembers opponents outside the FBS list.
    func game(fromESPN e: [String: Any], week fallbackWeek: Int) -> Game? {
        ...
        let week = ((e["week"] as? [String: Any])?["number"] as? Int) ?? fallbackWeek
    ...
    func loadSchedule(_ abbr: String) {
        ...
        fetch("\(Self.site)/teams/\(t.espnID)/schedule", cacheName: "schedule_\(t.espnID).json", maxAge: 1800) { [weak self] data in
            guard let self, let root = self.json(data) else { return }
            self.schedules[abbr] = (root["events"] as? [[String: Any]] ?? []).compactMap { self.game(fromESPN: $0, week: 0) }.sorted { $0.date < $1.date }
        }
    }
    ...
    private func announce(_ g: Game, team: String, points: Int) {
        ...
        showAlert(ScoreAlertInfo(kind: points >= 6 ? "TOUCHDOWN" : points == 3 ? "FIELD GOAL" : "SAFETY", team: team, points: points, game: g))
    }
    ...
    func showAlert(_ info: ScoreAlertInfo) {
        ...
        if inZone {
            scoreEvent = info
            NotificationCenter.default.post(name: Notification.Name("ZonesOpenZoneRequest"), object: nil, userInfo: ["title": "College Football", "detailed": true, "silent": true])
        }
        if alertStyle == "window" || alertStyle == "both" || !inZone { CFBScoreAlert.shared.show(info) }
    }
