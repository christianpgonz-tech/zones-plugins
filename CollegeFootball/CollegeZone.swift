...
@objc(CollegeFootballZone)
public final class CollegeFootballZone: NSObject {
    @objc public var zoneIdentifier: String { "college-football" }
    @objc public var zoneTitle: String { "College Football" }
    @objc public var zoneSymbol: String { "graduationcap.fill" }
    @objc public var zonePreferredWidth: NSNumber { 320 }
    @objc public var zonePreferredHeight: NSNumber { 236 }
    /// Zones (0.13.1 and later) shows this as a button under Settings → Added Zones → College Football.
    @objc public var zoneSettingsTitle: String { "College Football Settings…" }
    @objc public func zoneOpenSettings() { DispatchQueue.main.async { CollegeSettingsController.shared.show() } }
    /// Zones uses the first entry as this zone's starting position, so the list is ordered: free spots first.
    @objc public var zoneAllowedPlacements: [String] { CollegeFootballZone.placementOrder() }
    ...
    public override init() {
        ...
        DispatchQueue.main.async { CollegeStore.shared.start(); CFBChip.shared.start() }
    }
    ...
    static func placementOrder() -> [String] {
        ...
        if let saved = d.string(forKey: "cf.defaultPlacement"), preference.contains(saved) { return [saved] + preference.filter { $0 != saved } }
        var used = Set<String>()
        ...
        for (id, on) in enabledAdded where on && id != "added.college-football" { used.insert(placedAdded[id] ?? "top") }
        let pick = preference.first { !used.contains($0) } ?? "top"
        d.set(pick, forKey: "cf.defaultPlacement")
    ...
    @objc public func makeViewWithContext(_ context: NSDictionary) -> NSView {
        ...
        let hit = CFBHitMap()
        let host = NSHostingView(rootView: CollegeView(placement: placement, hit: hit))
        host.sizingOptions = []
        let box = CFBPassThroughBox(hit: hit)
