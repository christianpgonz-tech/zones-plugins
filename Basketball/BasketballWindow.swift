...
final class BasketballDetailsController {
    static let shared = BasketballDetailsController()
    private var window: NSWindow?
    let selection = BasketballSelection()

    func show(team: String, game: Game?) {
        selection.team = team
        selection.gameID = game?.id
        selection.tab = game?.state == .live ? .live : .scores
        BasketballStore.shared.loadSchedule(team)
        if window == nil {
            ...
            if !w.setFrameUsingName("BasketballDetailsWindow") { w.center() }
            w.setFrameAutosaveName("BasketballDetailsWindow")
            window = w
        }
        BasketballStore.shared.refreshBoard()
        NSApp.activate(ignoringOtherApps: true)
        window?.makeKeyAndOrderFront(nil)
    }
}

enum BasketballTab: String, CaseIterable, Identifiable { case scores = "Season", standings = "Standings", stats = "Team stats", live = "Live game"; var id: String { rawValue } }

final class BasketballSelection: ObservableObject {
    @Published var team = ""
    @Published var gameID: String?
    @Published var tab: BasketballTab = .scores
}

struct DetailsView: View {
    @ObservedObject var selection: BasketballSelection
    ...
    var body: some View {
        ...
        VStack(spacing: 12) {
            ...
            Picker("", selection: $selection.tab) { ForEach(BasketballTab.allCases) { Text($0.rawValue).tag($0) } }.pickerStyle(.segmented).labelsHidden()
            Group {
            ...
            HStack {
                ...
                Spacer()
                Text("Unofficial. Not affiliated with the NBA or any team. Data: ESPN's public ESPN's public NBA feed.")
...
extension BasketballSelection {
    /// Switches the whole window to another school, on its season page.
    func open(_ abbr: String) {
        team = abbr
        BasketballStore.shared.loadSchedule(abbr)
        gameID = BasketballStore.shared.currentGame(for: abbr)?.id
        tab = .scores
    }
}
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
                        ...
                        tile("Games played", "\(results.count)")
                    }
...
/// Choose your schools on the dial of every nba team; they become the rotating cards on the zone.
final class BasketballSettingsController {
    static let shared = BasketballSettingsController()
    private var window: NSWindow?
    func show() {
        if window == nil {
            ...
            w.title = "NBA Basketball Settings"
            w.isReleasedWhenClosed = false
            w.minSize = NSSize(width: 940, height: 680)
            w.contentView = NSHostingView(rootView: BasketballSettingsView())
            w.center()
            window = w
        }
        BasketballStore.shared.start()
        NSApp.activate(ignoringOtherApps: true)
        window?.makeKeyAndOrderFront(nil)
    }
}
        ...
        HStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("While one of your teams is playing (or about to), a thin dial by the zone's icon shows how far into the game it is, who leads, and when it's a close game.").font(.caption).foregroundStyle(.secondary)
                    Divider()
                    ...
                    if !BasketballStore.appCanOpenZones && store.alertStyle != "window" {
                    ...
                    Button("Preview alert") {
                        let a = store.favorites.first ?? store.teams.first?.abbr ?? "BOS"
                        var g = Game(id: "preview", date: Date(), state: .live, away: store.teams.first { $0.abbr != a }?.abbr ?? "NYK", home: a)
                        g.homeScore = 88; g.awayScore = 86; g.period = BasketballSport.regulation; g.clock = "1:12"; g.clockSeconds = 72
                    ...
                    Divider()
                    Text("Unofficial. Not affiliated with the NBA or any team. Schedules, scores, standings and rankings come from ESPN's public ESPN's public NBA feed.").font(.caption).foregroundStyle(.secondary)
