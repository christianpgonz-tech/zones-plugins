...
enum BasketballMode { case cards, games, picker }

final class BasketballUI: ObservableObject {
    init(mode: BasketballMode = .cards) { self.mode = mode }
    @Published var mode: BasketballMode
...
final class BasketballHitMap { var contains: (CGPoint) -> Bool = { _ in true } }

final class BasketballPassThroughBox: NSView {
    let hit: BasketballHitMap
    init(hit: BasketballHitMap) { self.hit = hit; super.init(frame: .zero) }
...
struct BasketballView: View {
    ...
    init(placement: String, hit: BasketballHitMap, startMode: BasketballMode = .cards) {
        self.placement = placement; self.hit = hit
        _ui = StateObject(wrappedValue: BasketballUI(mode: startMode))
    }
    @StateObject private var ui: BasketballUI
            ...
            ZStack(alignment: .topLeading) {
                if ui.mode == .picker {
                    ...
                    if ui.mode != .picker, let spot = SportBadgeLayout.spot(geo: geo, rect: rect) {
                        SportBadge(title: "NBA Basketball", size: spot.size) {
                            if let icon = BasketballIcon.image { Image(nsImage: icon).renderingMode(.template).resizable().scaledToFit() }
...
private struct PanelCanvas: View {
    @ObservedObject var ui: BasketballUI
    @ObservedObject var store: BasketballStore
    var body: some View {
        VStack(spacing: 6) {
            topBar
            Group {
    ...
    private var topBar: some View {
        HStack(spacing: 4) {
            ...
            Button { ui.mode = .picker } label: { Image(systemName: "square.grid.3x3.fill").font(.system(size: 11)) }
                .buttonStyle(.plain).help("Every NBA team: pick your favorites")
            Button { BasketballSettingsController.shared.show() } label: { Image(systemName: "gearshape").font(.system(size: 12)) }
                .buttonStyle(.plain).help("NBA Basketball settings")
        }.frame(height: 22)
    }
    private func tab(_ title: String, _ mode: BasketballMode) -> some View {
    ...
    @ViewBuilder private var cards: some View {
        ...
        if let game = store.currentGame(for: abbr) {
            GameCard(game: game, focus: abbr, celebrate: ui.celebration?.team)
                ...
                .onTapGesture { BasketballDetailsController.shared.show(team: abbr, game: game) }
...
struct TimeBar: View {
    let game: Game
    var body: some View {
        let n = BasketballSport.regulation
        VStack(spacing: 2) {
            GeometryReader { proxy in
            ...
            HStack {
                ForEach(1...n, id: \.self) { q in
                    Text(BasketballSport.periodNames[q - 1]).font(.system(size: 8, weight: game.state == .live && game.period == q ? .bold : .regular))
                        .foregroundStyle(game.state == .live && game.period == q ? Color.primary : .secondary).frame(maxWidth: .infinity)
                }
                if game.period > n { Text(BasketballSport.extraLabel).font(.system(size: 8, weight: .bold)).foregroundStyle(.orange) }
            }
        }
    }
}
...
private struct GameRow: View {
    ...
    var body: some View {
        ...
        .contentShape(Rectangle())
        .onTapGesture(count: 2) { BasketballDetailsController.shared.show(team: store.favorites.first { game.involves($0) } ?? game.home, game: game) }
...
enum BasketballIcon {
    static let image: NSImage? = {
        guard let url = Bundle(for: BasketballZone.self).url(forResource: "ZoneIcon", withExtension: "png") else { return nil }
