...
struct ScoreAlertInfo: Identifiable {
    let id = UUID()
    let kind: String            // "LEAD CHANGE", "SCORING RUN", "FINAL"
...
final class BasketballScoreAlert {
    static let shared = BasketballScoreAlert()
    ...
    func show(_ info: ScoreAlertInfo) {
        ...
        let view = ScoreAlertView(info: info, seconds: seconds, start: Date(), close: { [weak self] in self?.hide() },
                                  open: { [weak self] in self?.hide(); BasketballDetailsController.shared.show(team: info.team, game: info.game) })
        if panel == nil {
            let p = BasketballAlertPanel(contentRect: NSRect(origin: .zero, size: size), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
    ...
    private func target(_ size: CGSize) -> NSRect {
        let handle = NSApp.windows.first { $0.title == "NBA Basketball" && $0.isVisible && $0.frame.width <= 96 && $0.frame.height <= 96 }
...
final class BasketballAlertPanel: NSPanel {
...
struct ScoreAlertView: View {
    ...
    /// A plain summary of what happened.
    private func playText(_ g: Game) -> String {
        let name = store.team(info.team)?.name ?? info.team
        switch info.kind {
        case "FINAL": return "\(name) win by \(info.points)."
        case "SCORING RUN": return "\(name) on a \(info.points)-point run. It's \(g.away) \(g.awayScore), \(g.home) \(g.homeScore)."
        default: return "\(name) take the lead, \(g.away) \(g.awayScore), \(g.home) \(g.homeScore)."
        }
    }
}
