...
final class BasketballChip {
    static let shared = BasketballChip()
    private var panel: NSPanel?
    private var timer: Timer?
    private let state = BasketballXChipState()
    ...
    private func handle() -> NSWindow? { NSApp.windows.first { $0.title == "NBA Basketball" && $0.isVisible } }

    private func tick() {
        let store = BasketballStore.shared
        ...
        let side = 2 * (R + 34)
        if panel == nil {
            let p = BasketballXChipPanel(contentRect: NSRect(x: 0, y: 0, width: side, height: side), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
            p.isOpaque = false; p.backgroundColor = .clear; p.hasShadow = false; p.ignoresMouseEvents = true
            p.isReleasedWhenClosed = false
            p.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]
            p.contentView = NSHostingView(rootView: BasketballXChipView(state: state))
            panel = p
        }
...
final class BasketballXChipPanel: NSPanel {
...
final class BasketballXChipState: ObservableObject {
...
struct BasketballXChipView: View {
    @ObservedObject var state: BasketballXChipState
    var body: some View {
        GeometryReader { proxy in
            ...
            let reg = BasketballSport.regulation, n = Double(reg)
