enum BasketballSport {
    static let regulation = 4
    static let periodSeconds = 720.0
    static let periodNames = ["Q1", "Q2", "Q3", "Q4"]
...
struct Game: Identifiable, Hashable {
    ...
    var isClutch: Bool { state == .live && period >= BasketballSport.regulation && abs(homeScore - awayScore) <= 5 }
    /// "Q3", "2H", "OT", "2OT".
    var periodLabel: String {
        if period > BasketballSport.regulation { return period - BasketballSport.regulation == 1 ? "OT" : "\(period - BasketballSport.regulation)OT" }
        return BasketballSport.periodNames[max(1, period) - 1]
    }
    ...
    var progress: Double {
        ...
        case .post: return 1
        case .live:
            if period > BasketballSport.regulation { return 1 }
            let length = BasketballSport.periodSeconds
            return max(0, min(1, (Double(max(period, 1) - 1) * length + (length - clockSeconds)) / (length * Double(BasketballSport.regulation))))
        }
    }
    var statusLine: String {
        switch state {
        case .pre: return Game.kickoff.string(from: date)
        case .post: return overtime || period > BasketballSport.regulation ? "Final/OT" : "Final"
        case .live:
            if isDelayed { return "Delayed" }
            if period == BasketballSport.regulation / 2 && clockSeconds <= 0 && statusText.lowercased().contains("half") { return "Halftime" }
...
/// Everything the zone shows comes from ESPN's public ESPN's public NBA feed (free, unofficial): teams and conferences,
...
final class BasketballStore: ObservableObject {
    static let shared = BasketballStore()
    ...
    // Settings (saved)
    @Published var favorites: [String] { didSet { UserDefaults.standard.set(favorites, forKey: "nb.favorites"); loadSchedules() } }
    @Published var showLogos: Bool { didSet { UserDefaults.standard.set(showLogos, forKey: "nb.logos") } }
    @Published var rotateSeconds: Double { didSet { UserDefaults.standard.set(rotateSeconds, forKey: "nb.rotate") } }
    @Published var chipEnabled: Bool { didSet { UserDefaults.standard.set(chipEnabled, forKey: "nb.chip") } }
    /// Alerts: "off", "lead" (lead changes late in the game, and final scores) or "all" (also big scoring runs).
    @Published var alertMode: String { didSet { UserDefaults.standard.set(alertMode, forKey: "nb.alertMode") } }
    @Published var alertOthers: Bool { didSet { UserDefaults.standard.set(alertOthers, forKey: "nb.alertOthers") } }
    @Published var alertSeconds: Double { didSet { UserDefaults.standard.set(alertSeconds, forKey: "nb.alertSeconds") } }
    /// "zone" (the zone itself opens bigger), "window" (a separate card), or "both".
    @Published var alertStyle: String { didSet { UserDefaults.standard.set(alertStyle, forKey: "nb.alertStyle") } }
    ...
    private let fixtures: URL? = ProcessInfo.processInfo.environment["NBA_FIXTURES"].map { URL(fileURLWithPath: $0) }
    static var appCanOpenZones: Bool { (Int(Bundle.main.infoDictionary?["BasketballundleVersion"] as? String ?? "") ?? 0) >= 20 }
    static let boardQuery = ""
    private static let site = "https://site.api.espn.com/apis/site/v2/sports/basketball/nba"
    private static let apiV2 = "https://site.api.espn.com/apis/v2/sports/basketball/nba"
    static let conferenceOrder = ["Atlantic", "Central", "Southeast", "Northwest", "Pacific", "Southwest"]


    init() {
        let d = UserDefaults.standard
        favorites = d.stringArray(forKey: "nb.favorites") ?? []
        showLogos = d.object(forKey: "nb.logos") as? Bool ?? true
        rotateSeconds = d.object(forKey: "nb.rotate") as? Double ?? 8
        chipEnabled = d.object(forKey: "nb.chip") as? Bool ?? true
        alertMode = d.string(forKey: "nb.alertMode") ?? "lead"
        alertOthers = d.object(forKey: "nb.alertOthers") as? Bool ?? false
        alertSeconds = d.object(forKey: "nb.alertSeconds") as? Double ?? 10
        alertStyle = d.string(forKey: "nb.alertStyle") ?? "zone"
    }
    ...
    static var cacheFolder: URL {
        ...
        let url = base.appendingPathComponent("Zones/Basketball", isDirectory: true)
    ...
    private func fetch(_ urlString: String, cacheName: String, maxAge: TimeInterval, completion: @escaping (Data?) -> Void) {
        ...
        request.setValue("Zones-Basketball/1.0", forHTTPHeaderField: "User-Agent")
    ...
    private static func shortConference(_ name: String) -> String { name }
            ...
            self.fetch("\(Self.apiV2)/standings?level=3", cacheName: "standings.json", maxAge: 1800) { data in
                ...
                func walk(_ node: [String: Any], conference: String) {
                    if let entries = (node["standings"] as? [String: Any])?["entries"] as? [[String: Any]] {
                        let group = node["name"] as? String ?? conference
                ...
                for i in tables.indices { tables[i].rows.sort { ($0.wins - $0.losses, $0.diff) > ($1.wins - $1.losses, $1.diff) } }
    ...
    func game(fromESPN e: [String: Any]) -> Game? {
        ...
        g.overtime = g.period > BasketballSport.regulation
        return g
    }
    ...
    private func detectScores(_ games: [Game]) {
        for g in games where g.state != .pre {
            ...
            if let now, let before, now != before, g.period >= BasketballSport.regulation / 2 + (BasketballSport.regulation > 2 ? 1 : 0) {
    ...
    func showAlert(_ info: ScoreAlertInfo) {
        ...
        if inZone {
            scoreEvent = info
            NotificationCenter.default.post(name: Notification.Name("ZonesOpenZoneRequest"), object: nil, userInfo: ["title": "NBA Basketball", "detailed": true, "silent": true])
        }
        if alertStyle == "window" || alertStyle == "both" || !inZone { BasketballScoreAlert.shared.show(info) }
    }
...
struct TeamLogo: View {
    @ObservedObject var store = BasketballStore.shared
    let abbr: String
    var size: CGFloat = 40
    var body: some View {
