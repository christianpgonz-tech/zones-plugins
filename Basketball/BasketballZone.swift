...
@objc(BasketballZone)
public final class BasketballZone: NSObject {
    @objc public var zoneIdentifier: String { "nba-basketball" }
    @objc public var zoneTitle: String { "NBA Basketball" }
    ...
    /// Zones (0.13.1 and later) shows this as a button under Settings → Added Zones → NBA Basketball.
    @objc public var zoneSettingsTitle: String { "NBA Basketball Settings…" }
    @objc public func zoneOpenSettings() { DispatchQueue.main.async { BasketballSettingsController.shared.show() } }
    /// Zones uses the first entry as this zone's starting position, so the list is ordered: free spots first.
    @objc public var zoneAllowedPlacements: [String] { BasketballZone.placementOrder() }
    ...
    public override init() {
        ...
        DispatchQueue.main.async { BasketballStore.shared.start(); BasketballChip.shared.start() }
    }
    ...
    static func placementOrder() -> [String] {
        ...
        if let saved = d.string(forKey: "nb.defaultPlacement"), preference.contains(saved) { return [saved] + preference.filter { $0 != saved } }
        var used = Set<String>()
        ...
        for (id, on) in enabledAdded where on && id != "added.nba-basketball" { used.insert(placedAdded[id] ?? "top") }
        let pick = preference.first { !used.contains($0) } ?? "top"
        d.set(pick, forKey: "nb.defaultPlacement")
    ...
    @objc public func makeViewWithContext(_ context: NSDictionary) -> NSView {
        ...
        let host = NSHostingView(rootView: BasketballView(placement: placement, hit: hit))
        host.sizingOptions = []
        let box = BasketballPassThroughBox(hit: hit)
