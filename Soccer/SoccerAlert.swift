...
struct ScoreAlertInfo: Identifiable {
    let id = UUID()
    let kind: String            // "GOAL", "PENALTY GOAL", "OWN GOAL", "RED CARD"
    let team: String            // team key of the club concerned
    let detail: String?         // "E. Haaland 67'"
    let match: Match
}

/// A card that opens beside the zone's icon on a goal, then closes itself. It is its own floating window, so it never has to
/// resize the zone.
final class SCScoreAlert {
    static let shared = SCScoreAlert()
    ...
    func show(_ info: ScoreAlertInfo) {
        ...
        let view = ScoreAlertView(info: info, seconds: seconds, start: Date(), close: { [weak self] in self?.hide() },
                                  open: { [weak self] in self?.hide(); SoccerDetailsController.shared.show(team: info.team, match: info.match) })
        if panel == nil {
            let p = SCAlertPanel(contentRect: NSRect(origin: .zero, size: size), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
    ...
    private func target(_ size: CGSize) -> NSRect {
        let handle = NSApp.windows.first { $0.title == "Soccer" && $0.isVisible && $0.frame.width <= 96 && $0.frame.height <= 96 }
...
final class SCAlertPanel: NSPanel {
...
struct ScoreAlertView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                ...
                Text((team?.short ?? "").uppercased()).font(.system(size: 14, weight: .bold)).opacity(0.9).lineLimit(1)
                Spacer()
            ...
            VStack(spacing: 8) {
                ...
                if let d = info.detail { Text(d).font(.system(size: 14, weight: .semibold)).lineLimit(1) }
                Text("\(League.named(m.league).name) · \(m.statusLine)").font(.system(size: 11, weight: .semibold)).foregroundStyle(.secondary)
    ...
    private func side(_ key: String, _ score: Int) -> some View {
        let scored = key == info.team
        return VStack(spacing: 3) {
            TeamLogo(key: key, size: 46).shadow(color: scored ? (store.team(key)?.color ?? .white) : .clear, radius: 9)
