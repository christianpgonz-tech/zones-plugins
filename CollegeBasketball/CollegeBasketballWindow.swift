...
final class CollegeBasketballDetailsController {
    static let shared = CollegeBasketballDetailsController()
    private var window: NSWindow?
    let selection = CollegeBasketballSelection()

    func show(team: String, game: Game?) {
        selection.team = team
        selection.gameID = game?.id
        selection.tab = game?.state == .live ? .live : .scores
        CollegeBasketballStore.shared.loadSchedule(team)
        if window == nil {
            ...
            if !w.setFrameUsingName("CollegeBasketballDetailsWindow") { w.center() }
            w.setFrameAutosaveName("CollegeBasketballDetailsWindow")
            window = w
        }
        CollegeBasketballStore.shared.refreshBoard()
        NSApp.activate(ignoringOtherApps: true)
        window?.makeKeyAndOrderFront(nil)
    }
}

enum CollegeBasketballTab: String, CaseIterable, Identifiable { case scores = "Season", standings = "Standings", top25 = "Top 25", stats = "Team stats", live = "Live game"; var id: String { rawValue } }

final class CollegeBasketballSelection: ObservableObject {
    @Published var team = ""
    @Published var gameID: String?
    @Published var tab: CollegeBasketballTab = .scores
}

struct DetailsView: View {
    @ObservedObject var selection: CollegeBasketballSelection
    ...
    var body: some View {
        ...
        VStack(spacing: 12) {
            ...
            Picker("", selection: $selection.tab) { ForEach(CollegeBasketballTab.allCases) { Text($0.rawValue).tag($0) } }.pickerStyle(.segmented).labelsHidden()
            Group {
            ...
            HStack {
                ...
                Spacer()
                Text("Unofficial. Not affiliated with the NCAA or any school. Data: ESPN's public ESPN's public college basketball feed.")
...
extension CollegeBasketballSelection {
    /// Switches the whole window to another school, on its season page.
    func open(_ abbr: String) {
        team = abbr
        CollegeBasketballStore.shared.loadSchedule(abbr)
        gameID = CollegeBasketballStore.shared.currentGame(for: abbr)?.id
        tab = .scores
    }
}
            ...
            ScrollView {
                VStack(spacing: 2) {
                    HStack {
                        ...
                        ForEach(["Conf", "Overall", "PF/G", "PA/G", "Diff", "Strk"], id: \.self) { Text($0).font(.caption2).foregroundStyle(.secondary).frame(width: 52) }
                    }
                    ...
                    ForEach(Array(rows.enumerated()), id: \.element.id) { i, r in
                        HStack {
                            ...
                            Text(r.conf).frame(width: 52); Text(r.overall).frame(width: 52); Text(r.pfAvg).frame(width: 52); Text(r.paAvg).frame(width: 52)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
                        ...
                        tile("Points per game", s.pfAvg, "\(s.pf) in all")
                        tile("Allowed per game", s.paAvg, "\(s.pa) in all")
...
/// Choose your schools on the dial of every division i school; they become the rotating cards on the zone.
final class CollegeBasketballSettingsController {
    static let shared = CollegeBasketballSettingsController()
    private var window: NSWindow?
    func show() {
        if window == nil {
            ...
            w.title = "College Basketball Settings"
            w.isReleasedWhenClosed = false
            w.minSize = NSSize(width: 940, height: 680)
            w.contentView = NSHostingView(rootView: CollegeBasketballSettingsView())
            w.center()
            window = w
        }
        CollegeBasketballStore.shared.start()
        NSApp.activate(ignoringOtherApps: true)
        window?.makeKeyAndOrderFront(nil)
    }
}

struct CollegeBasketballSettingsView: View {
    @ObservedObject var store = CollegeBasketballStore.shared
        ...
        HStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("While one of your schools is playing (or about to), a thin dial by the zone's icon shows how far into the game it is, who leads, and when it's a close game.").font(.caption).foregroundStyle(.secondary)
                    Divider()
                    ...
                    if !CollegeBasketballStore.appCanOpenZones && store.alertStyle != "window" {
                    ...
                    Picker("Alert me for", selection: $store.alertMode) {
                        Text("Don't show").tag("off"); Text("Lead changes late in the game, and finals").tag("lead"); Text("Also big scoring runs").tag("all")
                    }
                    ...
                    Button("Preview alert") {
                        let a = store.favorites.first ?? store.teams.first?.abbr ?? "DUKE"
                        var g = Game(id: "preview", date: Date(), state: .live, away: store.teams.first { $0.abbr != a }?.abbr ?? "UNC", home: a)
                        g.homeScore = 88; g.awayScore = 86; g.period = CollegeBasketballSport.regulation; g.clock = "1:12"; g.clockSeconds = 72
                        store.showAlert(ScoreAlertInfo(kind: "LEAD CHANGE", team: a, points: 2, game: g))
                    }
                    Text("In the zone, it opens bigger on the card with a banner, then settles back. The separate window is a card beside the zone's icon. Scores are noticed within about 20 seconds.").font(.caption).foregroundStyle(.secondary)
                    Divider()
                    Text("Unofficial. Not affiliated with the NCAA or any school. Schedules, scores, standings and rankings come from ESPN's public ESPN's public college basketball feed.").font(.caption).foregroundStyle(.secondary)
