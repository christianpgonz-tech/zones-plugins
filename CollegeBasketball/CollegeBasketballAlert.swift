...
final class CollegeBasketballScoreAlert {
    static let shared = CollegeBasketballScoreAlert()
    ...
    func show(_ info: ScoreAlertInfo) {
        ...
        let view = ScoreAlertView(info: info, seconds: seconds, start: Date(), close: { [weak self] in self?.hide() },
                                  open: { [weak self] in self?.hide(); CollegeBasketballDetailsController.shared.show(team: info.team, game: info.game) })
        if panel == nil {
            let p = CollegeBasketballAlertPanel(contentRect: NSRect(origin: .zero, size: size), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
    ...
    private func target(_ size: CGSize) -> NSRect {
        let handle = NSApp.windows.first { $0.title == "College Basketball" && $0.isVisible && $0.frame.width <= 96 && $0.frame.height <= 96 }
...
final class CollegeBasketballAlertPanel: NSPanel {
