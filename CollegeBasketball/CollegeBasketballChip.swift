...
final class CollegeBasketballChip {
    static let shared = CollegeBasketballChip()
    private var panel: NSPanel?
    private var timer: Timer?
    private let state = CollegeBasketballXChipState()
    ...
    private func handle() -> NSWindow? { NSApp.windows.first { $0.title == "College Basketball" && $0.isVisible } }

    private func tick() {
        let store = CollegeBasketballStore.shared
        ...
        let side = 2 * (R + 34)
        if panel == nil {
            let p = CollegeBasketballXChipPanel(contentRect: NSRect(x: 0, y: 0, width: side, height: side), styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
            p.isOpaque = false; p.backgroundColor = .clear; p.hasShadow = false; p.ignoresMouseEvents = true
            p.isReleasedWhenClosed = false
            p.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]
            p.contentView = NSHostingView(rootView: CollegeBasketballXChipView(state: state))
            panel = p
        }
        ...
        default: let m = max(0, Int(g.date.timeIntervalSinceNow / 60)); label = m > 0 ? "in \(m)m" : "Tip-off"
        }
        state.update(placement: placement, radius: R, progress: g.progress, live: g.state == .live,
                     lead: mine > theirs ? 1 : (mine < theirs ? -1 : 0), redZone: g.isClutch,
                     hasBall: false, label: label)
        panel?.orderFrontRegardless()
    }
}

final class CollegeBasketballXChipPanel: NSPanel {
...
final class CollegeBasketballXChipState: ObservableObject {
...
struct CollegeBasketballXChipView: View {
    @ObservedObject var state: CollegeBasketballXChipState
    var body: some View {
        GeometryReader { proxy in
            ...
            let reg = CollegeBasketballSport.regulation, n = Double(reg)
            let gap = 0.045, w = geo.span / n
            ...
            let currentQuarter = min(reg - 1, Int(state.progress * n))
            TimelineView(.animation(minimumInterval: 1 / 20, paused: !state.redZone)) { ctx in
                ...
                ZStack {
                    ForEach(0..<reg, id: \.self) { i in
                        let a0 = start + w * Double(i) + gap / 2, a1 = start + w * Double(i + 1) - gap / 2
                        let fill = max(0, min(1, state.progress * n - Double(i)))
                    ...
                    if state.live {
                        let q = min(reg - 1, Int(state.progress * n))
                        let a = start + w * Double(q) + gap / 2 + (w - gap) * max(0, min(1, state.progress * n - Double(q)))
                    ...
                    HStack(spacing: 3) {
                        if state.hasBall { Image(systemName: "basketball.fill").font(.system(size: 8)) }
