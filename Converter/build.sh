#!/bin/bash
# Builds Converter.zone and Converter.zip (the file you put on your website).
set -e
cd "$(dirname "$0")"
NAME=Converter
rm -rf build && mkdir -p build/$NAME.zone/Contents/MacOS
SRC="../Shared/SportsPicker.swift ConverterData.swift ConverterModel.swift ConverterDial.swift ConverterViews.swift ConverterSettings.swift ConverterNetworking.swift ConverterNumberSystems.swift ConverterZone.swift"
...
mkdir -p build/$NAME.zone/Contents/Resources && cp ZoneIcon.png build/$NAME.zone/Contents/Resources/ZoneIcon.png
