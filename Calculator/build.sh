#!/bin/bash
# Builds Calculator.zone and Calculator.zip (the file you put on your website).
set -e
cd "$(dirname "$0")"
NAME=Calculator
...
swiftc -emit-library -module-name $NAME -o build/$NAME.zone/Contents/MacOS/$NAME \
  -Xlinker -bundle -target arm64-apple-macos14.0 CalculatorZone.swift
swiftc -emit-library -module-name $NAME -o build/x86 -Xlinker -bundle -target x86_64-apple-macos14.0 CalculatorZone.swift
